...
Usage:
    # Single file (PyTorch)
    python asr_model.py --model ./results-moonshine-fr-no-curriculum/final --audio sample.wav

    # ONNX Runtime (super fast!)
    python asr_model.py --model ./model-onnx --audio sample.wav --onnx

    # Manual ONNX (fastest)
    python asr_model.py --model ./model-onnx --audio sample.wav --use-manual-onnx

    # Directory of files
    python asr_model.py --model ./results-moonshine-fr-no-curriculum/final --audio ./test_audio/ --output ./transcriptions.json

    # Live transcription from microphone
    python asr_model.py --model ./model --live

    # With custom generation parameters
    python asr_model.py --model ./model --audio audio.wav --num-beams 5 --repetition-penalty 1.3
"""
...
class ManualONNXInference:
    ...
    def __init__(self, model_dir: str):
        ...
        # Detect model dimensions (layers, heads, head_dim)
        self._detect_dimensions()

        print(f"✅ Manual ONNX model loaded (layers={self.num_layers}, heads={self.num_heads}, head_dim={self.head_dim})")

    def _detect_dimensions(self):
        """Detect model dimensions from config.json or ONNX session."""
        # Default values (Tiny)
        self.num_layers = 6
        self.num_heads = 8
        self.head_dim = 36
        ...
        # 2. Fallback: Detect from ONNX session inputs
        try:
            inputs = self.decoder_session.get_inputs()
            kv_inputs = [inp for inp in inputs if "past_key_values" in inp.name]

            if kv_inputs:
                # Count distinct layer indices
                layer_indices = set()
                ...
                # Get head_dim and num_heads from first KV input shape
                # Shape is usually [batch, num_heads, sequence, head_dim]
            ...
            print(f"Warning: Could not detect model dimensions from ONNX session: {e}")
    ...
    def decode_greedy(self, encoder_hidden_states: np.ndarray, max_new_tokens: int = 50) -> np.ndarray:
        ...
        past_kv  = {}
        use_cache = False  # first step
        encoder_kv = None  # will store cross-attn KV once

        for i in range(num_layers):
            past_kv[f"past_key_values.{i}.decoder.key"] = np.zeros((B, num_heads, 0, head_dim), dtype=np.float32)
            past_kv[f"past_key_values.{i}.decoder.value"] = np.zeros((B, num_heads, 0, head_dim), dtype=np.float32)

            past_kv[f"past_key_values.{i}.encoder.key"] = np.zeros((B, num_heads, T_enc, head_dim), dtype=np.float32)
            past_kv[f"past_key_values.{i}.encoder.value"] = np.zeros((B, num_heads, T_enc, head_dim), dtype=np.float32)

        # Get decoder input names
        decoder_input_names = [inp.name for inp in self.decoder_session.get_inputs()]
        
        encoder_kv = { # compute only 1 - constant
                k : v for k, v in past_kv.items()
                if "encoder" in k
            }

        for _ in range(max_new_tokens):
            
            # 0. Prepare decoder inputs
            decoder_inputs = {}
            ...
            decoder_inputs["use_cache_branch"] = np.array([use_cache], dtype=bool)
            #1. Run decoder
            decoder_inputs.update(past_kv) # adding the past-kv for re-using the attention computation
        
            # print(f"type of decoder_inputs {type(decoder_inputs)}")
            # print(f"{decoder_inputs.keys()}")
            outputs = self.decoder_session.run(None, decoder_inputs) # infer & decode the next-token based on the previous tokens information
            logits = outputs[0]
            kv_outputs = outputs[1:] # raw-tensor not being labeled - unacceptable in ONNX
            # layer 0 [dec_k, dec_v, enc_k, enc_v] -> len(kv_out)= #layer * 4 = 6 * 4= 24

            # 2. PARSING KV
            new_decoder_kv = {}
            new_encoder_kv = {}
            idx = 0 # itterating through layer

            # 3. Build KV dict safely
            for i in range(num_layers):
                
                # decoder self-attention KV
                dec_k = kv_outputs[idx]; idx +=1
                dec_v = kv_outputs[idx]; idx +=1

                # cross-att KV
                enc_k = kv_outputs[idx]; idx +=1
                enc_v = kv_outputs[idx]; idx +=1 

                new_decoder_kv[f"past_key_values.{i}.decoder.key"] = dec_k
                new_decoder_kv[f"past_key_values.{i}.decoder.value"] = dec_v

                # ONLY store encoder KV once
                if encoder_kv is None:
                    new_encoder_kv[f"past_key_values.{i}.encoder.key"] = enc_k
                    new_encoder_kv[f"past_key_values.{i}.encoder.value"] = enc_v
            
            # 4. persist the KV
            if encoder_kv is None:
                encoder_kv = new_encoder_kv

            # 5. Assign
            past_kv = {}
            past_kv.update(new_decoder_kv)
            past_kv.update(encoder_kv)
            ...
            use_cache = True  # after first step
...
class LiveTranscriber:
    ...
    def _callback_with_vad(self, indata, frames, time_info, status):
        ...
        # accumulate buffer
        self.vad_buffer = np.append(self.vad_buffer, audio)

        # Silero requires >= 512 samples at 16kHz
        min_len = 512

        while len(self.vad_buffer) >= min_len:
            frame = self.vad_buffer[:min_len]
            self.vad_buffer = self.vad_buffer[min_len:]

            try:
                speech_dict = self.vad_iterator(frame, return_seconds=True)
            ...
            if speech_dict and 'start' in speech_dict:
                self.audio_buffer = np.copy(frame)
...
Examples:
  # Single file
  python asr_model.py --model ./results-moonshine-fr/final --audio sample.wav

  # Directory of files
  python asr_model.py --model ./model --audio ./test_audio/ --output results.json

  # Custom generation parameters
  python asr_model.py --model ./model --audio audio.wav \\
      --num-beams 5 --repetition-penalty 1.3 --no-repeat-ngram-size 2

  # GPU with FP16
  python asr_model.py --model ./model --audio audio.wav --device cuda --fp16

  # Live transcription with VAD (recommended)
  python asr_model.py --model ./model --live

  # Live transcription without VAD (continuous mode)
  python asr_model.py --model ./model --live --no-vad --chunk-duration 2.0

  # ONNX Runtime for super fast inference (requires ONNX model)
  python asr_model.py --model ./model-onnx --audio audio.wav --onnx

  # Manual ONNX (fastest)
  python asr_model.py --model ./model-onnx --audio audio.wav --use-manual-onnx
        """
    )
...
# python src/tools/asr_model.py \
# >  --model resources/asr/moonshine_model/onnx/merged/tiny-vi/float \
# > --live \
# > --use-manual-onnx